...
module.exports.addDeck = async function (req, res, next){
     ...
     //update User
     //await owner.save()
     await User.findByIdAndUpdate(newDeck.owner, { $push : { decks : newDeck._id }  })     
...
module.exports.replaceDeck = async function (req, res, next){
     ...
     //Old Owner pull id
     await User.findByIdAndUpdate(ownerId, { $pull: { decks: { $in: [deck._id] } } });
...
module.exports.deleteDeck = async function (req, res, next){
     ...
     await User.findByIdAndUpdate(owner._id, { $pull : { decks : { $in : [deck._id] } } })
